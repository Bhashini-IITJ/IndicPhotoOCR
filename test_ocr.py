...
def preprocess_image(image_path):
    try:
        # Load image with reduced size initially
        img = cv2.imread(image_path, cv2.IMREAD_REDUCED_GRAYSCALE_2)  # Downscale by factor of 2
        if img is None:
            print(f"Warning: Failed to load image: {image_path}")
            return image_path, False
        # Apply threshold
        img = cv2.threshold(img, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)[1]
        cv2.imwrite(image_path, img)  # Overwrite with processed image
...
def perform_ocr(image_path):
    try:
        ...
        if isinstance(result, list):
            ...
            return " ".join(str(item) for item in flattened if item)
    ...
    if os.path.exists(json_file):
        try:
            ...
            for locality_id, locality_data in data.items():
                ...
                filename = locality_data.get("filename", "Unknown")
                ...
                image_path = os.path.join(image_directory, mapped_filename)
                if not os.path.exists(image_path):
                    print(f"Warning: Image {mapped_filename} not found for Locality ID {locality_id}. WRR set to 0%.")
                    ocr_text = ""
                    wrr = 0.0
                else:
